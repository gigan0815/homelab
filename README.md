...
A single-node Proxmox setup running self-hosted services for personal and family

use. The focus is on learning by doing — setting up, maintaining, and eventually

monitoring real infrastructure rather than just reading about it.
...
| Storage | 2× 3TB HDD · ZFS mirror via USB enclosure |
...
Infrastructure is managed via Ansible. The control node runs as a dedicated LXC

container on the Proxmox host.
...
\- `paperless\_backup.yml` — exports paperless-ngx and syncs to OneDrive
...
| paperless-ngx | OneDrive | Daily 02:00 | document\_exporter + rclone |



Storage redundancy is provided via a hardware RAID mirror on the USB enclosure

housing the 2× 3TB drives.
